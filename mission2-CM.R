...
#calculate two week rolling average and add to df
df<-df %>% group_by(date) %>% 
  mutate(rolling_avg = rollmean(value, k = 2, fill = NA, align = "right")/2) %>% 
  ungroup()
...
plot <- df %>%
  ...
  geom_point(na.rm = T) +
  geom_line(na.rm = T) +
  #add a two week moving average
  geom_line(aes(y=rolling_avg), color="black", linetype="dashed") +
  theme_classic()
