...
df_site <- read.csv2("./data/Belgium_export-site.csv", sep = ";") %>%
  select(siteName, date, value, value_load, value_pmmv, value_avg14d_past, value_load_avg14d_past, value_pmmv_avg14d_past)
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    # Bottom acknowledgment box
    div(class = "info-box",
        "Acknowledgment: Thanks to all the sites providing us with wastewater samples" )
